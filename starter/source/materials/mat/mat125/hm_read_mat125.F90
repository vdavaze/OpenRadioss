...
#include  "my_real.inc"
 ...
 !-----------------------------------------------
        ...
        integer, intent(in)                          :: maxfunc   !< size of functions table
        integer, intent(in)                          :: npropm    !< number of material parameters
        integer, intent(in)                          :: iout      !< output unit number
        integer, intent(inout)                       :: nfunc     !< number of temporary variables
        integer, dimension(maxfunc), intent(inout)   :: ifunc     !< function table
        type(mlaw_tag_), intent(inout)               :: mtag      !< material law tag
        my_real, dimension(100),intent(inout)        :: parmat    !< local material parameters
        type (unit_type_),intent(in)                 :: unitab    !< table of input units
        my_real, dimension(npropm) ,intent(inout)    :: pm        !< material parameters
        type(submodel_data), dimension(*),intent(in) :: lsubmodel !< submodel table
 ...
 !-----------------------------------------------
        ...
        my_real ::                                                             &
         ...
         ef22t,ef22c,ef33t,ef33c,fac,fcut,efs,ms,als,                          &
...
!card17 - Strain rate dependency (optional)    
...
!
        !< Checking poisson's ratio consistency
        nu12 = nu21*e1/e2 
        nu23 = nu32*e2/e3
        nu13 = nu31*e1/e3
        if (nu12*nu21 >= one) then
          call ancmsg(msgid=3068,                                              &
                    ...
                    i1=mat_id,                                                 &
                    c1=titr)
        elseif (nu13*nu31 >= one) then
          call ancmsg(msgid=3069,                                              &
                    msgtype=msgerror,                                          &
                    anmode=aninfo_blind_2,                                     &
                    i1=mat_id,                                                 &
                    c1=titr)
        elseif (nu23*nu32 >= one) then
...
!
        !< Check determinant of 2D compliance matrix
        detc = one - nu12*nu21
        if (detc <= zero) then
        ...
        !< Stiffness matrix for 2d plane stress
        fac = one/detc
        a11 = e1*fac
        a12 = nu21*a11
        a22 = e2*fac
        ...
        c13 = -nu31/e3
        c23 = -nu23/e2      
        !< Check determinant of 3D compliance matrix
...
!     
        ...
        allocate (matparam%uparam(matparam%niparam))
        allocate (matparam%uparam(matparam%nuparam))

        !< Material integer parameters
        matparam%niparam(1) = fs

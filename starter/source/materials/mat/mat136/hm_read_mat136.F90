...
!
          ...
          lambda_m = e*nu*hthick/(one - nu*nu)
          mu_m     = e*hthick/(two*(one + nu))
          !< Bending elastic parameters
          lambda_b = e*nu*hthick**3/(12.0d0*(one - nu*nu))
          mu_b     = e*hthick**3/(24.0d0*(one + nu))
          !< Shear modulus
          shear = e/(two*(one+nu))
          !< Bulk modulus
...
!
          ...
          matparam%bulk      = bulk
          matparam%uparam(1) = hthick
          matparam%uparam(2) = lambda_m
          matparam%uparam(3) = mu_m
          matparam%uparam(4) = lambda_b
          matparam%uparam(5) = mu_b
          matparam%uparam(6) = tshear
!
          !< PARMAT table
          parmat(1)  = bulk
          parmat(2)  = e
...
1003      format(/                                                                 &
            ...
            5X,"CONCRETE THICKNESS (H). . . . . . . . . . . . . . . . .=",1PG20.13/)
!
